...
for(s in 1:length(species.list_2021)){
  ...
  scores <- join_scores(dt$conf_pred_90, dt$conf_pred_95, dt$conf_pred_99)
  labels <- join_scores(dt$correct, dt$correct, dt$correct)
  dt.c <- mmdata(scores, labels, modnames = c("90", "95", "99"))
  
  scores.r <- join_scores(dt$r_pred_90, dt$r_pred_95, dt$r_pred_99)
  labels.r <- join_scores(dt$correct, dt$correct, dt$correct)
  dt.r <- mmdata(scores.r, labels.r, modnames = c("90", "95", "99"))
  ...
  ## PRAUC values
  output_2021$PRAUC_90.c[s] <- pr_auc[1]
  output_2021$PRAUC_95.c[s] <- pr_auc[2]
  output_2021$PRAUC_99.c[s] <- pr_auc[3]
  
  output_2021$PRAUC_90.r[s] <- pr_auc.r[1]
  output_2021$PRAUC_95.r[s] <- pr_auc.r[2]
  output_2021$PRAUC_99.r[s] <- pr_auc.r[3]
